...
<div class="max-w-5xl mx-auto">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-200 p-10">
        <h2 class="text-3xl font-bold text-gray-900 mb-6">Edit Event</h2>
        @if($errors->any())
        ...
        <form method="POST" action="{{ route('admin.events.update', $event) }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="organizer" value="{{ old('organizer', $event->organizer) }}" required class="w-full px-5 py-3 border border-gray-300 rounded-3xl focus:ring-2 focus:ring-blue-500 outline-none text-sm text-gray-700">
                </div>
                <div>
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $event->title) }}" required class="w-full px-5 py-3 border border-gray-300 rounded-3xl focus:ring-2 focus:ring-blue-500 outline-none text-sm text-gray-700">
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Tanggal *</label>
                    <input type="date" name="event_date" value="{{ old('event_date', $event->event_date->format('Y-m-d')) }}" required class="w-full px-5 py-3 border border-gray-300 rounded-3xl focus:ring-2 focus:ring-blue-500 outline-none text-sm text-gray-700">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Waktu *</label>
                    <input type="time" name="event_time" value="{{ old('event_time', $event->event_time) }}" required class="w-full px-5 py-3 border border-gray-300 rounded-3xl focus:ring-2 focus:ring-blue-500 outline-none text-sm text-gray-700">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Lokasi *</label>
                    <input type="text" name="location" value="{{ old('location', $event->location) }}" required class="w-full px-5 py-3 border border-gray-300 rounded-3xl focus:ring-2 focus:ring-blue-500 outline-none text-sm text-gray-700">
                </div>
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Deskripsi *</label>
                <textarea name="description" rows="5" required class="w-full px-5 py-3 border border-gray-300 rounded-3xl focus:ring-2 focus:ring-blue-500 outline-none text-sm text-gray-700">{{ old('description', $event->description) }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Persyaratan</label>
                <textarea name="requirements" rows="3" class="w-full px-5 py-3 border border-gray-300 rounded-3xl focus:ring-2 focus:ring-blue-500 outline-none text-sm text-gray-700">{{ old('requirements', $event->requirements) }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Upload Gambar Baru</label>
